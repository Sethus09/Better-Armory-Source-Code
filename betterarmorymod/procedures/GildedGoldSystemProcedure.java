...
public class GildedGoldSystemProcedure {
...
if (entity == null || source == null) return;
    ...
    if (source.getEntity() instanceof Player) { // Cheque al jugador
        ...
        if (player.getMainHandItem().getItem() instanceof net.setho.betterarmorymod.item.GildedSwordItem) {
            // Probabilidad de un lingote de oro
            if (world.random.nextDouble() < 0.25) { // 25% de probabilidad
                entity.spawnAtLocation(new ItemStack(Items.GOLD_INGOT));
            }
        }
    }
}
